...
Overshoot_target =  PITCH_D/50
ts_target = 600
Waveform_oscillation_bound = 1
Static_error_target = PITCH_D/50
adjust_bound = 0.02
belief_times = 50
...
class PID_model():
    ...
    def get_epsolid_reward(self,env, k1=1.5, k2=2.5, k3=0.5):
        ...
        Waveform_oscillation = np.var(error_list)
        if if_alpha:
            reward += 4500
            done = False
        # 目前返回：done, ts, OS, SE
    ...
    def model_simulation_fliter( self,k1=1.5, k2=2.5, k3=0.5, total_step=800):
        '''
        按照当前参数，模拟total_step步，并且绘制theta图
        :param k1: kp
        :param k2: kd
        ...
        while i < total_step:
            ...
            # CHANGE：简单的加权均值滤波
            if len(dez_list) >= 2:
                action = 0.5*action + 0.3*dez_list[-1] + 0.2*dez_list[-2]
            dez_list.append(action)
            # 采取动作
            self.env.step(np.array([action]))
            alpha.append(self.env.arfa * 57.3)
        ...
        plt.plot(time, desired_theta, label="time-desired_theta")
        plt.legend(loc="best")
        plt.title("This is the K epoch with fliter")
        plt.show()
        return alpha, dez_list, theta, desired_theta, error_list
...
class RL_PI2:
    ...
    def policy_evl(self):
        ...
        for j in range(roll_outs):
            data=self.cal_trajectory_loss(j)
            # K的变化值
            self.k_delta[0, j] = data[0]
            self.k_delta[1, j] = data[1]
            self.k_delta[2, j] = data[2]
            # 当前K值
            self.K_roll[0, j] = data[3]
            self.K_roll[1, j] = data[4]
            self.K_roll[2, j] = data[5]
            self.loss[j] = data[6]
        '''
    """ -----------------------------------------------------------策略改善------------------------------------------------------------"""
    def policy_improve(self):
        ...
        last_reward,last_done,last_ts,last_overshoot,last_static_error = self.reward_model.get_epsolid_reward(self.env,self.K[0],self.K[1],self.K[2])
        self.K = self.K + temp_k
        cur_reward,cur_done,cur_ts,cur_overshoot,cur_static_error = self.reward_model.get_epsolid_reward(self.env,self.K[0],self.K[1],self.K[2])
        # 方差更新
        type_sigma[0] = (last_overshoot < cur_overshoot)
        type_sigma[1] = (last_overshoot < cur_overshoot)
        type_sigma[2] = (last_static_error < cur_static_error or  last_ts < cur_ts)
        # self.Loss_end = np.array([loss[2].min(), loss[3].min(), loss[4].min()])
        for id_ in range(3):
    ...
    def training(self):
        ...
        while i < training_times:
            ...
            if self.current_training % self.attenuation_step_length == 0  and self.current_training != 0:
                # TODO：不应该这么暴力。应该让sigma与reward挂钩
                ...
                if self.current_training %3 == 0:
                    ...
                    plt.title("The action without fliter")
                    plt.plot(dez_list1, color='red', label='without fliter')
                    plt.show()
