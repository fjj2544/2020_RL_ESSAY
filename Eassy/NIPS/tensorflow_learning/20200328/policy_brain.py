...
class Policy_Net():
    # 初始化网络
    def __init__(self, observation_dim, action_dim, action_bound,
                 lr=1e-5, model_file=None):
        ...
        self.action_bound = action_bound  # 动作限幅,动作限幅
        # 采样参数
        ...
        with tf.variable_scope("Actor"):
            ...
            # 动作
            self.action = 3 * tf.layers.dense(inputs=self.policy_2, units=self.n_actions, activation=tf.nn.tanh,
                                               trainable=True)
            # self.action  = tf.multiply(self.out1,self.action_bound[1])
            # 网络参数
            self.pi_params = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope="Actor")
        # 采样
        self.normal_dist = tf.contrib.distributions.Normal(self.action, self.sigma_exploration)
        self.sample = tf.clip_by_value(tf.squeeze(self.normal_dist.sample(1), axis=0), self.action_bound[0],
                                       self.action_bound[1])
    ...
    def learn(self, batch_obs, batch_target_act, epoch=10):
        for i in range(epoch):
            ...
            loss = self.sess.run([self.loss],
                                 feed_dict={self.input_layer: batch_obs, self.target_act: batch_target_act})
        # print(loss)
        # print(loss)
        return float(loss[0])
    ...
    # 重建网络
    def restore_model(self, model_path=save_model_path):
