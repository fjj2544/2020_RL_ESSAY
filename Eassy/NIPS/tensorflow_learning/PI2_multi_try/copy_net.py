...
class Policy_Net():
    ...
    def __init__(self, env, name, lr=0.0001, model_file=None , trainable=True):
        ...
        self.old_p ,old_p_params = self._build_net(name="old_p",trainable=True)
        # self.pi,pi_params = self._build_net(name="pi",trainable=False)
        # self.update_oldpi_op = [oldp.assign(p) for p, oldp in zip(pi_params,old_p_params)]
...
if __name__ == '__main__':
    ...
    res1 = net1.sess.run(net1.old_p,feed_dict={net1.input_layer : obs})
    print(res1)
    # res2 = net1.sess.run(net1.old_p,feed_dict={net1.input_layer : obs})
