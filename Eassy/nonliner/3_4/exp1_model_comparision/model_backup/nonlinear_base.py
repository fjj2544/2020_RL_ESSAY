...
class PID_model():
    ...
    def get_new_env(self, env,start_step,simulation_step = 5000,k1=1.5, k2=2.5, k3=0.5):
        ...
        return self.env,alpha, dez_list, pitch, desired_pitch,q_list,height_list,mh_list,error_list,ierror_list,derror_list,mz0_list,mz_list,Mz_list,Jz_list,lift_list,Drag_list,theta_list,desired_theta_list,true_theta_list,true_theta_desire_list,domga_z_list
...
class RL_PI2:
    ...
    def rolling_optimization(self, rolling_interval=250, total_step=5000):
        ...
        while iterator < opt_times:
            ...
            self.env,alpha, delta_z, pitch, pitch_desired,q,height,mh,error,ierror,derror,mz0,mz,Mz,Jz,lift,drag,theta,theta_desire,true_theta,true_theta_desire,domga_z = self.reward_model.get_new_env(self.env, self.cur_step, rolling_interval, self.K[0], self.K[1], self.K[2])
