...
class Planes_Env:
    ...
    def step(self,action):
        ...
        mz = mz0 + mz_e + mz_a + mzz * 57.3*self.omega_z * self.Lr / (2 * self.Mach * self.Vs)
        # 升力
        ...
        self.Jz = self.Jz + 99.635*self.dmass
...
def target_func(cur_step):
    y = 1.5*math.sin(cur_step  * math.pi / 2000)/57.3
    return y
def target_func_dot(cur_step):
    y_dot = 1.5*(math.pi* math.sin(cur_step * math.pi / 2000) )/ (57.3*2000)
    return y_dot
'''------------------------------PID控制器板块------------------------------'''
...
class PID_model():
    ...
    def get_epsolid_reward(self,env,start_step,simulation_step = 5000, k1=1.5, k2=2.5, k3=0.5):
        ...
        for i in range(simulation_step):
            ...
            self.env.dpithch_desired = target_func_dot(cur_step=start_step+i)
            ...
            # 计算损失
            if i == 0:
                loss3 = float(100*abs(action-self.action))
            loss1 += math.fabs(error)
            # loss2 += float(0.001 * action**2)
        ...
        if if_alpha:
            loss4 = max(loss1 + loss2 + loss3, 4500)
        print("损失的数量级：")
...
class RL_PI2:
    ...
    def rolling_optimization(self, rolling_interval=250, total_step=1000):
        ...
        worksheet = workbook.add_sheet('action')
        worksheet.write(0, 0, 'P')
        worksheet.write(0, 1, 'D')
        worksheet.write(0, 2, 'I')
        i = 1
        for data in action_p:
            temp = float(data)
            worksheet.write(i,0,temp)
            i = 1 + i
        i = 1
        for data in action_d:
            temp = float(data)
            worksheet.write(i, 1, temp)
            i = 1 + i
        i = 1
        for data in action_i:
            temp = float(data)
            worksheet.write(i, 2, temp)
            i = 1 + i
        workbook.save('action.xls')
        plt.figure(2)
