...
class Planes_Env:
    ...
    def __init__(self):
        ...
        # 目标
        self.Mach_desire = 7.3
        self.dMach_desire = 0.0
...
class PID_model():
    ...
    # 进行一次PID模拟
    def get_epsolid_reward(self,env,cur_target_desire,cur_target_factor,cur_step,total_step,simulation_step = 5000, k1=1.5, k2=2.5, k3=0.5):
        self.env = copy.deepcopy(env)
        action_list = []
        ierror = copy.deepcopy(self.ierror)
        last_Mach_desire = copy.deepcopy(self.last_Mach_desire)
        loss =0
        alpha_count = 0
        action_count = 0
        if_alpha =False
        ...
        for i in range(simulation_step):
            if cur_step + i <= total_step:
                self.env.Mach_desire =  cur_target_desire + cur_target_factor * i
            self.env.dMach_desire = (self.env.Mach_desire - last_Mach_desire)/self.env.tau
            last_Mach_desire =copy.deepcopy(self.env.Mach_desire)
            error =  (self.env.Mach_desire - self.env.Mach) * self.env.Vs
            derror = (self.env.dMach_desire - self.env.dMach) * self.env.Vs
    ...
    def get_new_env(self, env,cur_target_desire,cur_target_factor ,cur_step,total_step, simulation_step = 5000,k1=1.5, k2=2.5, k3=0.5):
        ...
        for i in range(simulation_step):
            if cur_step + i <= total_step:
                self.env.Mach_desire =  cur_target_desire + cur_target_factor * i
            self.env.dMach_desire = (self.env.Mach_desire - self.last_Mach_desire) / self.env.tau
            self.last_Mach_desire= copy.deepcopy(self.env.Mach_desire)
            self.error = (self.env.Mach_desire - self.env.Mach) * self.env.Vs
            self.derror = (self.env.dMach_desire - self.env.dMach) * self.env.Vs
            ...
            mh_list.append(self.env.Mach*self.env.Vs)
            mh_desire_list.append(self.env.Mach_desire*self.env.Vs)
    ...
    def model_simulation( self,env,start_step,simulation_step = 5000,k1=1.5, k2=2.5, k3=0.5):
        ...
        for i in range(simulation_step):
            # TODO:设计目标函数
            self.env.Mach_desire = target_func(cur_step=start_step + i)
            self.env.dMach_desire = (self.env.Mach_desire - last_Mach_desire) / self.env.tau
            last_Mach_desire = copy.deepcopy(self.env.Mach_desire)
            error = (self.env.Mach_desire - self.env.Mach) * self.env.Vs
            derror = (self.env.dMach_desire - self.env.dMach) * self.env.Vs
            ...
            time.append(i)
            mach.append(self.env.Mach)
            desired_mach.append(self.env.Mach_desire)
        plt.figure(2)
        plt.plot(time,mach , label="time-Mach")
        plt.plot(time, desired_mach, label="time-desired_Mach")
        plt.legend(loc="best")
...
class RL_PI2:
    ...
    def __init__(self,if_filter=RAND_INIT, attenuation_step_length=1, alpha=0.85):
        ...
        ## 记录当前被控量的目标值,滚动开始的时候
        self.cur_target_desire = 0
        ## 主要用来算目标的
        self.cur_target_factor = 0
        ## 记录总仿真步数
        self.total_step = 0
    ## 重新设置training 参数
    def reset_training_state(self):
        ...
        self.cur_loss = self.reward_model.get_epsolid_reward(self.env, self.cur_target_desire,self.cur_target_factor, self.cur_step,self.total_step,self.opt_steps, self.K[0], self.K[1], self.K[2])
    ...
    def cal_trajectory_loss(self, j):
        ...
        cur_loss = self.reward_model.get_epsolid_reward(self.env, self.cur_target_desire,self.cur_target_factor, self.cur_step,self.total_step,self.opt_steps, cur_k1, cur_k2, cur_k3)
    ...
    def policy_improve(self):
        ...
        self.cur_loss = self.reward_model.get_epsolid_reward(self.env,  self.cur_target_desire,self.cur_target_factor,self.cur_step,self.total_step, self.opt_steps, self.K[0], self.K[1],
                                                              self.K[2])
        # 确保K_opl的最优性
        ## 记录样本中最小的
    ...
    def rolling_optimization(self, rolling_interval=250, total_step=5000):
        ...
        iterator = 0
        max_time = 0
        self.cur_target_factor = ( target_func(0) - self.env.Mach)/total_step
        self.cur_target_desire = copy.deepcopy(self.env.Mach)
        while iterator < opt_times:
            self.cur_target_desire = self.env.Mach
            self.cur_target_factor = (target_func(iterator * rolling_interval) - self.env.Mach) / (total_step - rolling_interval * iterator)
            self.training()
            ...
            true_theta_desire,domga_z,dtheta,mh_desire = \
                self.reward_model.get_new_env(self.env, \
                                              self.cur_target_desire,self.cur_target_factor, self.cur_step,self.total_step,rolling_interval, self.K[0], self.K[1], self.K[2])
            ...
            for item in mh_desire:
                mh_desire_list.append(item)
            if iterator % 1==0:
                plt.figure(2)
                plt.plot(mh_list, 'b+', label="time-mach")
                plt.plot(mh_desire_list, 'r', label="time-desired_mach")
                plt.legend(loc="best")
        ...
        plt.plot(mh_desire_list,'r',label="time-desired_mh")
        plt.legend(loc="best")
