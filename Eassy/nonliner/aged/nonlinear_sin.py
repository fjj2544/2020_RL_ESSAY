...
'''跟踪目标函数'''
def target_func(cur_step,total_step =5000):
    if cur_step < total_step/2:
        return 3*(math.sin(cur_step  * math.pi / 2000)) / (cur_step  + 2) + (math.sin(cur_step  * math.pi / 500)) * (
            math.cos(cur_step  * math.pi / 2000)) / 57.3
    if cur_step  >= total_step / 2:
        return  2*(3+ (1 / (1 + np.exp(cur_step  / total_step - math.cos(cur_step  * math.pi / 500)))) + math.cos(
            cur_step * math.pi / 500) * math.sin(cur_step  * math.pi / 2000) ) / 57.3
'''------------------------------PID控制器板块------------------------------'''
...
class PID_model():
    def __init__(self):
        self.env = Planes_Env()
    # 进行一次PID模拟
    def get_epsolid_reward(self,env, k1=1.5, k2=2.5, k3=0.5,total_step = 5000,cur_step=0):
        '''
        进行PID模拟全程
        :param env: 环境
        :param k1: kp
        ...
        for i in range(total_step):
            if count >= belief_times and ts == total_step:
                ts = i
                break
            self.env.pitch_desired = target_func(cur_step=cur_step+i,total_step=5000)
        ...
        return loss_sin
    def get_new_env(self, env,step_time ,cur_step,k1=1.5, k2=2.5, k3=0.5):
        '''
        对环境进行更新
        :param env: 原环境
        ...
        while True:
            ...
            if i == step_time:
                break
            self.env.pitch_desired = target_func(cur_step=cur_step+i,total_step=total_step)
            error = (self.env.pitch_desired * 57.3 - self.env.observation[0])
            derror = self.env.dpithch_desired * 57.3 - self.env.observation[1]
            error_list.append(error)
            derror_list.append(derror)
            ...
            dez_list.append(self.env.altitude)
            self.env.step(np.array([action]))
            alpha.append(self.env.Mach)
            theta.append(self.env.observation[0])
            desired_theta.append(self.env.pitch_desired * 57.3)
            q.append(self.env.observation[1])
            time.append(i)
    ...
    def model_simulation( self,k1=1.5, k2=2.5, k3=0.5, total_step=800):
        ...
        while i < total_step:
            # TODO:设计目标函数
            self.env.pitch_desired = target_func(cur_step=i, total_step=total_step)
            error = (self.env.pitch_desired * 57.3 - self.env.observation[0])
            derror = self.env.dpithch_desired * 57.3 - self.env.observation[1]
            error_list.append(error)
            derror_list.append(derror)
...
class RL_PI2:
    ...
    def cal_trajectory_loss(self, j):
        ...
        loss = self.reward_model.get_epsolid_reward(self.env,cur_k1, cur_k2, cur_k3,total_step=self.opt_interval,cur_step=self.cur_step)
    ...
    def policy_improve(self):
        ...
        reward = self.reward_model.get_epsolid_reward(self.env, self.K[0],self.K[1],self.K[2],total_step=self.opt_interval)
        # 确保K_opl的最优性
        best_K_ind = np.argmin(loss)
        if loss[best_K_ind] < self.Loss_opl:
            self.K_opl[0] = self.K_roll[0, best_K_ind]
            self.K_opl[1] = self.K_roll[1, best_K_ind]
            self.K_opl[2] = self.K_roll[2, best_K_ind]
            self.Loss_opl = loss[best_K_ind]
    ...
    def training(self):
        ...
        self.loss_after_training[self.current_training] = self.reward_model.get_epsolid_reward(self.env, self.K[0],
                                                                                               self.K[1],self.K[2],total_step=self.opt_interval,cur_step=self.cur_step)
        self.Loss_opl = 1e10
        while i < training_times:
            ...
            if self.current_training % self.attenuation_step_length == 0 and self.current_training != 0:
                ...
                if self.current_training % 3 == 0:
                    ...
                    # 进行情况模拟
                    alpha, dez_list1, theta, desired_theta, error_list1 = self.reward_model.model_simulation(self.K[0], self.K[1], self.K[2], 10000)
                    # 比较两条曲线
            ...
            # 记录参数
            self.K_after_training[:, self.current_training] = self.K[:, 0]
            self.loss_after_training[self.current_training] = self.reward_model.get_epsolid_reward(self.env,self.K[0], self.K[1],self.K[2],cur_step=self.cur_step)
            # 判断是否可以停止
    ...
    # 滚动优化
    def rolling_optimization(self,rolling_time=250,total_step=5000):
        # 优化次数
        ...
        while iterator < opt_times:
            ...
            self.cur_step = iterator * rolling_time
            self.K_after_roll_step[:,iterator]=self.K[:,0]
            self.env,alpha, delta_z, theta, theta_desired = self.reward_model.get_new_env(self.env,rolling_time,self.cur_step,self.K[0],self.K[1],self.K[2])
            iterator += 1
            for item in alpha:
