...
class Planes_Env:
    ...
    def step(self, action):
        ...
        Isp = 4900
        #--------------------------动力学方程---------------------------
        # v变化[马赫/s] 速度除以Vs 转换为马赫,反过来就是速度,公式检查正确
        self.dMach = (P * np.cos(self.arfa) - Drag - self.mass * self.G0 * np.sin(self.theta)) / (self.mass * self.Vs)
        # 这里采用计算Y轴方向的速度[m/s],检查正确
        self.daltitude = self.Mach * self.Vs * np.sin(self.theta)
        # 飞行角速度,这个保持和word一致
        self.dtheta = (P * np.sin(self.arfa) + Lift -  self.mass * self.G0 * np.cos(self.theta)) / (self.mass * self.Mach * self.Vs)
        # 质量变化,这是一个常量
        self.dmass = -P / Isp
        # X轴方向速度
        self.drrange = self.Mach * self.Vs * np.cos(self.theta)
        # omega_z的变化,下面正确,但是角加速度没有限制?
        ...
        self.observation = np.array([self.pitch*57.3, self.dpitch*57.3,mz0,mz,Mz,self.Jz,Lift,Drag,self.theta*57.3,self.theta_desired*57.3,self.domega_z*57.3])
        return self.observation
...
def target_func(cur_step):
    y = 5 *  math.sin(cur_step  * math.pi / 1000)/57.3
    return y
...
class PID_model():
    ...
    def get_new_env(self, env,start_step,simulation_step = 5000,k1=1.5, k2=2.5, k3=0.5):
        ...
        for i in range(simulation_step):
            ...
            omag_z_list.append(self.env.observation[10])
        ...
        return self.env,alpha, dez_list, pitch, desired_pitch,q_list,height_list,mh_list,error_list,ierror_list,derror_list,mz0_list,mz_list,Mz_list,Jz_list,lift_list,Drag_list,theta_list,desired_theta_list,true_theta_list,true_theta_desire_list,omag_z_list
...
class RL_PI2:
    ...
    def rolling_optimization(self, rolling_interval=250, total_step=5000):
        ...
        while iterator < opt_times:
            ...
            self.env,alpha, delta_z, pitch, pitch_desired,q,height,mh,error,ierror,derror,mz0,mz,Mz,Jz,lift,drag,theta,theta_desire,true_theta,true_theta_desire,omag_z = self.reward_model.get_new_env(self.env, self.cur_step, rolling_interval, self.K[0], self.K[1], self.K[2])
        ...
        ## 绘制pitch曲线
        plt.plot(omag_z_list, 'b+', label="domgaz")
        plt.legend(loc="best")
        plt.title("domagz Graph After %d Opt" % iterator)
        # plt.show()
        save_figure(save_dir, "domagz.png")
        ###############绘制theta曲线
