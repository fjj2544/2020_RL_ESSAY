...
class RL_PI2:
    ...
    def policy_evl(self):
        '''
        进行20次并行的策略评估，并且记录在K_del和K_roll和loss中
        :return: 无
        '''
        # 并行进行N次策略评估，得到【3d,3k,1loss】
        multi_res = [poll.apply_async(self.get_trajectory_loss, (j,)) for j in range(roll_outs)]
        # 对当前的k参数进行更新——k_delta，K_roll，loss
    ...
    def iterator_finished(self):
        ...
        '''
        # 判断迭代的效果是不是应该结束
        flag1 = sum((self.K_after_training[:, self.current_training - 1] -
                     self.K_after_training[:,self.current_training]) ** 2) <= 1e-6
    ...
    def training(self):
        ...
        while i < training_times:
            ...
            self.loss_after_training[self.current_training] = self.reward_model.get_epsolid_reward(self.env,self.K[0], self.K[1],self.K[2])
    ...
    def rolling_optimization(self,rolling_time=20,total_step=200):
        '''
        进行滚动优化
        :param rolling_time: 滚动次数
        :param total_step: 总步长
        :return: 无
        '''
        # 滚动优化的次数
...
if __name__ == "__main__":
    ...
    # 更改的部分
    #env = Planes_Env()
    #reward_model.get_epsolid_reward(env)
    # 更改的部分完
    alpha_list =[]
    delta_z_list = []
    theta_list = []
