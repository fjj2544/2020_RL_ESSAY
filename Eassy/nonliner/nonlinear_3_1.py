...
# 强化学习
RAND_INIT = True
training_times = 15             # 训练次数
roll_outs = 20                  # 并行数量
Max_Adjust_times = 1000         # 最大调整次数
# 目标曲线
PITCH_DESIRED = []
for i in range(15000):
    PITCH_DESIRED.append(-0.0/57.3)

# for i in range(15000):
#     if i < 5000:
#         PITCH_DESIRED.append((1.5 + (math.sin(i * math.pi / 2000)) / (i + 2) + 2*(math.sin(i * math.pi / 500)) * (math.cos(i * math.pi / 2000)) )/ 57.3)
#     else:
#         PITCH_DESIRED.append((4 + (3 + (2 / (1 + np.exp(i / 10000 - math.cos(i * math.pi / 500)))) + math.cos(i * math.pi / 500) * math.sin(i * math.pi / 2000)) )/ 57.3)
...
class PID_model():
    def __init__(self):
        self.env = Planes_Env()
    # 进行一次PID模拟
    def get_epsolid_reward(self,env, start_time, k1=1.5, k2=2.5, k3=0.5, total_step=10000):
        '''
        进行PID模拟全程
        :param env: 环境
    ...
    def get_new_env(self, env, start_time, step_time ,k1=1.5, k2=2.5, k3=0.5):
        '''
        对环境进行更新
        :param env: 原环境
        ...
        while True:
            ...
            i = i + 1
        #TODO:必须把这个环境赋值给全局环境！
        return self.env,alpha, dez_list, theta, desired_theta,Mach_list, Altitude_list

    def model_simulation(self, env, start_time, k1=1.5, k2=2.5, k3=0.5, total_step=800):
        '''
        ...
        #plt.show()
        plt.savefig("This is the K epoch"+str(start_time)+".png")
        plt.cla()
...
class RL_PI2:
    ...
    def cal_trajectory_loss(self, j):
        ...
        #   CHECK!
        loss = self.reward_model.get_epsolid_reward(self.env, self.start_time, cur_k1, cur_k2, cur_k3, self.rolling_step)
    ...
    def policy_improve(self):
        ...
        reward, done, ts, Overshoot, Static_error = self.reward_model.get_epsolid_reward(self.env,self.start_time, self.K[0],self.K[1],self.K[2],self.rolling_step)
        # 确保K_opl的最优性
    ...
    def training(self):
        ...
        self.loss_after_training[self.current_training] = self.reward_model.get_epsolid_reward(self.env, self.start_time, self.K[0],self.K[1],self.K[2],self.rolling_step)[0]
        while i < training_times:
            ...
            if self.current_training % self.attenuation_step_length == 0 and self.current_training != 0:
                if self.current_training %5 == 0:
                    ...
                    plt.savefig("loss between"+str(self.current_training - 5)+"and "+str(self.current_training)+".png")
                    plt.cla()
                    print("当前的K和损失是")
                    print(self.K)
                    print(self.current_training)
                    print("----------------------------")
                    # 进行情况模拟
                    self.reward_model.model_simulation(self.env, self.start_time, self.K[0], self.K[1], self.K[2], self.rolling_step)
                    # 比较两条曲线
                    # alpha, dez_list2, theta, desired_theta, error_list2 = self.reward_model.model_simulation_fliter(self.K[0], self.K[1], self.K[2], 10000)
                    # print("原始标准差为："+str(np.std(error_list1))+"\t滤波标准差为："+str(np.std(error_list2)))
                    # det_err = np.array(error_list2) - np.array(error_list1)
                    #plt.title("The action with fliter")
                    #plt.plot(dez_list2, color='green', label='with fliter')
                    #plt.show()

                    # 判断是否K还原
                    if self.Loss_opl < self.loss_after_training[self.current_training]:
                        print("================Change the best one================")
                        self.K = self.K_opl
    ...
    # 滚动优化
    def rolling_optimization(self,rolling_time=1000 ,total_step=10000):
        opt_times = int(total_step/rolling_time)            # 优化次数
        theta_list = []
        theta_desired_list =[]
        alpha_list =[]
        delta_z_list = []
        ...
        iterator = 0
        self.env = Planes_Env()
        self.rolling_step = rolling_time
        ...
        while iterator < opt_times:
            ...
            print("<<<<<<<<<<<<<<<Rolling>>>>>>>>>>>>>>>")
            self.K_after_roll_step[:,iterator]=self.K[:,0]
            self.env,alpha, delta_z, theta, theta_desired,Mach, Altitude = self.reward_model.get_new_env(self.env, self.start_time,
                                                                                          rolling_time,self.K[0],self.K[1],self.K[2])
            self.start_time += rolling_time
            for item in Mach:
            ...
            if iterator % 2==0:
                ...
                plt.savefig("Rolling optimization graph"+str(iterator)+".png")
                plt.cla()
                plt.plot(mach_list, 'b', label="Mach")
                plt.title("Mach")
                plt.savefig("Mach"+str(iterator)+".png")
                plt.cla()
                plt.plot(altitude_list, 'b', label="Mach")
                plt.title("Altitude")
                plt.savefig("Altitude"+str(iterator)+".png")
                plt.cla()
        plt.figure(2)
        plt.plot(theta_list, 'b+', label="time-theta")
        plt.plot(theta_desired_list, 'r', label="time-desired_theta")
...
if __name__ == "__main__":
    ...
    alpha, delta_z, theta, theta_desired, k_after, loss, cur_time = model.rolling_optimization(rolling_time=1000,
                                                                                               total_step=10000)
    ...
    # alpha, delta_z, theta, theta_desired ,elist= reward_model.model_simulation(model.env, 0, k[0],k[1],k[2],1000)
    # alpha_list.append(alpha)
    # delta_z_list.append(delta_z)
    # theta_list.append(theta)
    # theta_desire_list.append(theta_desired)
