...
class PID_model():
    ...
    def get_epsolid_reward(self, k1=1.5, k2=2.5, k3=0.5):
        ...
        # print(type(abs(max(abs(np.array(theta))) - max(abs(np.array(desired_theta)))) + abs(error_list[-1])))
        print(type(abs(max(map(abs,theta)) - max(map(abs,desired_theta)))[0] ++ abs(error_list[-1])[0] ))
        return abs(max(abs(np.array(theta))) - max(abs(np.array(desired_theta))))[0] + abs(error_list[-1])
