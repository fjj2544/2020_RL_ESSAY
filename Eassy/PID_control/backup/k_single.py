    ...
    def training(self):
        for i in range(training_times):
            ...
            for j in range(roll_outs):
                ...
                # self.loss[j] = self.loss[j] * self.loss[j];
                # self.loss[j] = np.sqrt(self.loss[j])
            ...
            # self.loss_after_training[self.current_training] = np.power(self.loss_after_training[self.current_training],2)
            # self.loss_after_training[self.current_training] = np.sqrt(self.loss_after_training[self.current_training])
