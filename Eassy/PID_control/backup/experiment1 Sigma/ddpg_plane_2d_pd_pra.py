...
adjust_times = 1
class RL_PI2:
    def __init__(self,alpha=0.85,Iteration_step_length=1):
    ...
    def training(self):
        ...
        while i < training_times+adjust_times:
            ...
            # 方差衰减和可视化
            if self.current_training % self.Iteration_step_length == 0  :
        ...
        ##对比方便用于区间赋值
        for i in range(self.current_training,101):
            self.K_after_training[:, i] = self.K[:, 0]
            self.loss_after_training[i] = self.loss_after_training[self.current_training]
...
def get_data_from_exp():
    ...
    K1 = np.zeros((3, 1), dtype=np.float64)
    K2 = np.zeros((3, 1), dtype=np.float64)
    K3 = np.zeros((3, 1), dtype=np.float64)


    model = RL_PI2(alpha=1/0.85, Iteration_step_length=1)
    model.set_initial_value()
    K1[0],K1[1],K1[2],K_after_train,loss,train_time = model.training()
    max_train_time = max(max_train_time,train_time)
    loss_list.append(loss)
    K_after_train_list.append(K_after_train)
    save_data("./data/exp1/K_after_train/", "Iteration_step_length_1_K_after_train.txt", K_after_train)
    save_data("./data/exp1/Loss/", "Iteration_step_length_1_loss.txt", loss)
    save_data("./data/exp1/K/", "Iteration_step_length_1_K.txt", K1)

    model = RL_PI2(alpha=1/0.85, Iteration_step_length=5)
    model.set_initial_value()
    K2[0],K2[1],K2[2], K_after_train, loss,train_time = model.training()
    max_train_time = max(max_train_time,train_time)

    loss_list.append(loss)
    K_after_train_list.append(K_after_train)
    save_data("./data/exp1/K_after_train/", "Iteration_step_length_5_K_after_train.txt", K_after_train)
    save_data("./data/exp1/Loss/", "Iteration_step_length_5_loss.txt", loss)
    save_data("./data/exp1/K/", "Iteration_step_length_5_K.txt", K2)

    model = RL_PI2(alpha=1/0.85, Iteration_step_length=10)
    model.set_initial_value()
    K3[0],K3[1],K3[2], K_after_train, loss,train_time = model.training()
    max_train_time = max(max_train_time,train_time)

    loss_list.append(loss)
    K_after_train_list.append(K_after_train)
    save_data("./data/exp1/K_after_train/", "Iteration_step_length_10_K_after_train.txt", K_after_train)
    save_data("./data/exp1/Loss/", "Iteration_step_length_10_loss.txt", loss)
    save_data("./data/exp1/K/", "Iteration_step_length_10_K.txt", K3)
    ...
    save_data("./data/exp1/Alpha/", "Iteration_step_length_1_alpha.txt", alpha)
    save_data("./data/exp1/Delta/", "Iteration_step_length_1_delta_z.txt", delta_z)
    save_data("./data/exp1/Theta/", "Iteration_step_length_1_theta.txt", theta)
    save_data("./data/exp1/Theta/", "Iteration_step_length_1_theta_desire.txt", theta_desire)
    model.data_record()
    ...
    theta_list.append(theta)
    theta_desire_list.append(theta_desire)
    save_data("./data/exp1/Alpha/", "Iteration_step_length_5_alpha.txt", alpha)
    save_data("./data/exp1/Delta/", "Iteration_step_length_5_delta_z.txt", delta_z)
    save_data("./data/exp1/Theta/", "Iteration_step_length_5_theta.txt", theta)
    save_data("./data/exp1/Theta/", "Iteration_step_length_5_theta_desire.txt", theta_desire)
    ...
    theta_list.append(theta)
    theta_desire_list.append(theta_desire)
    save_data("./data/exp1/Alpha/", "Iteration_step_length_10_alpha.txt", alpha)
    save_data("./data/exp1/Delta/", "Iteration_step_length_10_delta_z.txt", delta_z)
    save_data("./data/exp1/Theta/", "Iteration_step_length_10_theta.txt", theta)
    save_data("./data/exp1/Theta/", "Iteration_step_length_10_theta_desire.txt", theta_desire)
    return alpha_list,delta_z_list,theta_list,theta_desire_list,K_after_train_list,loss_list,max_train_time
def plot_result(alpha_list,delta_z_list,theta_list,theta_desire_list):
    label = ["Iteration Step Length 1 ","Iteration Step Length 5","Iteration Step Length 10"]
    color = ["r","g","b","k"]
    line_style = ["-","--",":","-."]
    ...
    plt.show()
def plot_loss_k(K_after_training_list ,loss_after_training_list,train_time):
