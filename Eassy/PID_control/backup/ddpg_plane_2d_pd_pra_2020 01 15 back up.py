...
Overshoot_bound = 0.5
Waveform_oscillation_bound = 0.01
Static_error_bound = 0.1
# 波形震荡
...
class PID_model():
    ...
    def get_epsolid_reward(self, k1=1.5, k2=2.5, k3=0.5,is_test = False):
        ...
        weight = 100
        # 约束惩罚
        bound_cost = count*2500
        r =  weight/Overshoot_bound*Overshoot + weight/Static_error_bound*Static_error + weight/Waveform_oscillation_bound*Waveform_oscillation +bound_cost
        # if bound_cost == 0:
        #     print("I  FIND GOOD K")
        if is_test and bound_cost != 0:
            return False
        return r
...
class RL_PI2:
    def __init__(self):
        # 记录每次策略迭代之后的K(包括初始化）   动态变量
        self.K = np.zeros((3, 1), dtype=np.float64)
        # 记录每 roll_outs 局势内的 K  动态变量 每一局刷新一次
        ...
        # K变化量,用于存储噪声大小,进行策略改进        动态变量 每一局刷新一次
        ...
        # 也就是每次迭代完之后计算一下回报
        self.loss_after_training = np.zeros((training_times+1000, 1), dtype=np.float64)  # loss function after each training
        # 每次迭代都记录参数
        self.K_after_training = np.zeros((3, training_times+1000), dtype=np.float64)  # K after each training
    ...
    def test_res(self):
        if self.reward_model.get_epsolid_reward(self.K[0], self.K[1], self.K[2], True)==False:
          return False
        else:
            return True
    ...
    def training(self):
        ...
        while i < training_times:
            ...
            ## 增大鲁棒性
            flag = True
            multi_res = [poll.apply_async(self.test_res, ()) for j in range(10)]
            for res in multi_res:
                flag &= res.get()
            # 最优替换,个人认为最优替换huoduole
            if( flag == False and i == training_times -1):
                i -= 1
                self.current_training = training_times + adjust_times
                ## 自适应改变探索率
                self.sigma = self.sigma / self.alpha
                # self.reward_model.model_simulation(self.K[0], self.K[1], self.K[2],training_times+adjust_times)
                adjust_times += 1
            else:
                i+=1
            # print(i,self.K)
        ...
        plt.savefig("K.png")
        plt.show()
        plt.plot(self.loss_after_training[50:training_times+adjust_times])
