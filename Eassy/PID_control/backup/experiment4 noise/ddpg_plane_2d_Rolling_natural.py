...
""" -----------------------------------------------------------强化学习部分-------------------------------------------------------------"""
# 训练次数,也就是策略迭代次数
training_times = 10  # training times
# 路径数量,也就是策略评估数量
roll_outs = 20  # path number
# 后期可能加入延迟更新
Max_Adjust_times = 100
class RL_PI2:
    def __init__(self,IS_FILTER = True,attenuation_step_length=10,alpha=0.85):
        ## 滚动优化,用于记录当前阶段
        self.env = Planes_Env()
    ...
    def cal_trajectory_loss(self, j):
        ...
        if (self.Filter and self.current_training > 1 and loss > self.loss_after_training[self.current_training - 1]):
...
def plot_result(alpha_list,delta_z_list,theta_list,theta_desire_list,figure_number = 3):
    label = ["Filter PI2+ Rolling Optimization ","Filter PI2 ","Natural PI2","Reference parameter"]
    color = ["r","g","b","k"]
    line_style = ["-","-.",":","--"]
...
if __name__ == "__main__":
    ...
    ## 第1组优化10次
    training_times = 10
    model = RL_PI2(IS_FILTER=True,alpha=0.85,attenuation_step_length=1)
    ...
    # ## 第3组对照组
    training_times = 100
    model = RL_PI2(IS_FILTER=False,alpha=1/0.85,attenuation_step_length=10)
    model.set_initial_value()
    alpha, delta_z, theta, theta_desired = model.rolling_optimization(rolling_time=500, total_step=500)
    alpha_list.append(alpha)
    delta_z_list.append(delta_z)
